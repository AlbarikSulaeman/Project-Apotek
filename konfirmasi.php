...
session_start();
 ...
 include("koneksi.php");
 $item = mysqli_query($mysql, "SELECT * FROM tb_order WHERE status = 'Pengecekan Surat Dokter' ORDER BY tanggal ASC");
 $no = 1;
 ?>
<!DOCTYPE html>
<html lang="en">
      ...
      <th>Surat Dokter</th>
		...
		<tr>
			...
			while ($prod = mysqli_fetch_array($item)) {
    			...
    			echo "<td><a href='surdok.php?id=$prod[id]'>Surat Doktera</td>";
    			echo "<td><a  href='konfirmasi2.php?id=$prod[id]&&status=ditolak'><button class='btn btn-main'>Tolak</button></a>";
				echo "<td><a  href='konfirmasi2.php?id=$prod[id]&&status=diterima'><button class='btn btn-main'>Terima</button></a>";
			}
			?>
		</tr>
 	</table>
